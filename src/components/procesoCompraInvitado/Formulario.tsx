...
import { validateCompraInvitado } from "@/helpers/validacion-compra-invitado"
...
const FormularioCompra: React.FC<Props> = ({ myIP }) => {
    ...
    const handleSubmit = async (event: React.FormEvent) => {
        ...
        const erroresValidacion = await validateCompraInvitado(todos);
            ...
            const payload = {
                ...
                antifraud: {
                    ...
                    merchantDefineData: {
                        ...
                        MDD75: 'Registrado',
                        MDD77: 1, // Dias de registro desde que se creo el usuario hasta la fecha 
                    },
                },
                dataMap: {
                    cardholderCity: 'Lima',
    ...
    const procesarTransaccion = async (dataTransaccion: any, purchaseNumber: string) => {
        const urlParamsObject = {}
        const path = "visitante/order/store"
        const payLoad = {
            purchaseNumber: purchaseNumber,
            participante_id: todos.id,
            nombres: todos.nombres,
    ...
    const handleChange = (e: ChangeEvent<FormElement>) => {
        ...
        } else if (/^[a-zA-Z-ñ'-']*$/.test(value.replace(/\s/g, ''))) {
            setTodos(prevTodos => ({
                ...prevTodos,
                [name]: value
            }))
                                        ...
                                        <div className={styles.FormDatos}>
                                            ...
                                            <div>
                                                <input
                                                    ...
                                                    onChange={(e) => handleChangeMovil(e)}
                                                />
                                            </div>
                                            <div>
                                                <input
